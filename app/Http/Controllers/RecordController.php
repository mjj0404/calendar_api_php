...
class RecordController extends Controller
{
    ...
    public function getUserRecords(Request $request)
    {
        ...
        $records = Record::where('externid', '=', $externid)->get();
        return response()->json($records);
    }
    
    public function getUserRecordsByLunarDate(Request $request)
    {
        $user = Auth::user();
        $user = $request->user();
        $externid = $user->externid;
        ...
        $startSolar = new Solar();
        $startSolar->solarYear = date("Y");
        $startSolar->solarMonth = date("m");
        $startSolar->solarDay = date("d");
        ...
        $leapDayCheck = ((date("m") == 2 && date("d") == 29) ? true : false);
        $endSolar->solarDay = ($leapDayCheck ? $endSolar->solarDay - 1 : $endSolar->solarDay);
        $endSolarInt = LunarSolarConverter::SolarToInt($endSolar->solarYear, $endSolar->solarMonth, $endSolar->solarDay);

        $excludedList = (array) null;
        $sortedIdIntPair = (array) null;
        
        foreach($records as $record) {
            // convert calendarId integer value to Solar and then Lunar
            $solar = new Solar();
            $solar = LunarSolarConverter::SolarFromInt($record->calendarid);
            $lunar = LunarSolarConverter::SolarToLunar($solar);
            $lunar->lunarYear = date("Y");
            $solar = LunarSolarConverter::LunarToSolar($lunar);
            $solarInt1 = LunarSolarConverter::SolarToInt($solar->solarYear, $solar->solarMonth, $solar->solarDay);

            $lunar->lunarYear = date("Y") - 1;
            $solar = LunarSolarConverter::LunarToSolar($lunar);
            $solarInt2 = LunarSolarConverter::SolarToInt($solar->solarYear, $solar->solarMonth, $solar->solarDay);
            
            if ($solarInt1 >= $startSolarInt && $solarInt1 <= $endSolarInt) {
                if (sizeof($sortedIdIntPair) == 0) {
                    $sortedIdIntPair = array(array(
                        'recordid'=>$record->recordid,
                        'solarInt'=>$solarInt1
                    ));
                }
                else {
                    array_push($sortedIdIntPair,array(
                        'recordid'=>$record->recordid,
                        'solarInt'=>$solarInt1
                        )
                    );
            ...
            else if ($solarInt2 >= $startSolarInt && $solarInt2 <= $endSolarInt) {
                if (sizeof($sortedIdIntPair) == 0) {
                    $sortedIdIntPair = array(array(
                        'recordid'=>$record->recordid,
                        'solarInt'=>$solarInt2
                ...
                array_push($excludedList, $record->recordid);
            }
        };
        
        $records = $records->except($excludedList);
        
        // sort the associative array by 'solarInt' so that 
        // recordid can be ordered by time that's not in the table column
        usort($sortedIdIntPair, function ($item1, $item2) {
            return $item1['solarInt'] <=> $item2['solarInt'];
        });


        // extract only recordid from the associative array
        $sortedRecordId = (array) null;
        foreach($sortedIdIntPair as $item) {
            array_push($sortedRecordId, $item['recordid']);
        }

        // order records by the sortedRecordId
        $records = $records->sortBy(function($record) use ($sortedRecordId) {
            return array_search($record->recordid, $sortedRecordId);
        });

        $collection = collect(new Record);
        foreach($records as $record) {
